...
class CategoryController:
    def __init__(self):
        ...
        # Save images under dashboard assets (so UI can load via ./assets/..)
        # project_root / dashboard / assets / uploads / categories
        project_root = Path(__file__).resolve().parents[2]
        self.upload_dir = project_root / "dashboard" / "assets" / "uploads" / "categories"
        self.upload_dir.mkdir(parents=True, exist_ok=True)

    def _save_dataurl_image(self, data_url: str) -> str:
        """
        data_url example: data:image/png;base64,xxxx
        returns DB image_path: assets/uploads/categories/<file>
        """
        ...
        return f"assets/uploads/categories/{filename}"
    ...
    def update(self, category_id: int, payload: dict):
        ...
        try:
            # if new image provided, save new and remove old
            if payload.get("image_base64"):
                old = self.repo.get_image_path(int(category_id))  # add repo method below
